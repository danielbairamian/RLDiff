...
def generate_rollout(env, ppo_agent, deterministic=False):
    ...
    b_action_means = torch.zeros((T, B, 1), device=env.device)      # α / (α+β) — where
    b_concentrations = torch.zeros((T, B, 1), device=env.device)    # α + β     — how confident
    final_x0s = None
    ...
    # rollout loop
    for t in range(T):
        action, value, logprob, conc_alpha, conc_beta = ppo_agent(obs['x0'], obs['alpha'], obs['steps'] / env.budget, deterministic=deterministic)
        next_obs, rewards, dones = env.step(action.squeeze(-1))

        b_states[t] = obs['x0']
        b_alphas[t] = obs['alpha']
        b_steps[t] = obs['steps'] / env.budget
        b_actions[t] = action
        b_logprobs[t] = logprob
        b_rewards[t] = rewards
        b_dones[t] = dones
        b_values[t] = value
        b_action_means[t] = conc_alpha / (conc_alpha + conc_beta)   # interpretable mean
        b_concentrations[t] = conc_alpha + conc_beta                # interpretable confidence
    ...
    b_concentrations = b_concentrations[:last_t]
    ...
    rollout = {key: val[active_mask] for key, val in {
        ...
        'action_means': b_action_means, 'concentrations': b_concentrations,
    }.items()}
...
def ppo_update(ppo_agent, minibatch_size=64, full_rollout=None):
    for i in range(0, full_rollout['states'].shape[0], minibatch_size):
        ...
        new_logprobs, new_values, entropy, conc_alpha, conc_beta = ppo_agent.evaluate_actions(minibatch['states'], minibatch['alphas'], minibatch['steps'], minibatch['actions'])
...
if __name__ == "__main__":
    ...
    iadb_model = torch.load(os.path.join(diffusion_path, f'iadb_model.pth'), map_location=device).eval()
    ...
    env = DiffusionEnv(dataloader=dataloader, iadb_model=iadb_model, device=device, order=args.order, budget=args.budget, sample_multiplier=args.sample_multiplier, denorm_fn=denorm_fn)
    env.reset()
    ppo_agent = PPOAgent(vision_encoder=vision_encoder, 
                         ...
                         concentration_init=2.0).to(device)
