...
class PPOAgenNthOrder(nn.Module):
    def __init__(self, vision_encoder:VisionEncoder, state_dim:int, fused_dims:int, time_encoder_dims:List[int], projection_dims:List[int], action_dim:int, act_min:float=0.0, act_max:float=1.0, std_init:float=0.3, mean_action_init:float=0.1):
        super(PPOAgenNthOrder, self).__init__()
        ...
        with torch.no_grad():
            # 1. Initialize weights normally around 0
            self.action_mean.weight.normal_(0, 0.01)

            # 2. Initialize Biases with Normal distributions centered at targets
            # Head 0: Step Size -> Normal(mean=mean_action_init, std=0.01)
            self.action_mean.bias[0].normal_(mean=mean_action_init, std=0.01)
            
            # Head 1: Order -> Normal(mean=0.5, std=0.01)
            self.action_mean.bias[1].normal_(mean=0.5, std=0.01)
