...
def generate_rollout(env, ppo_agent, deterministic=False):
    ...
    b_order_actions = torch.zeros((T, B, 1), device=env.device)
    ...
    b_order_means = torch.zeros((T, B, 1), device=env.device) # for debugging and analysis purposes
    b_action_log_stds = torch.zeros((T, B, 1), device=env.device) # for debugging and analysis purposes
    b_order_log_stds = torch.zeros((T, B, 1), device=env.device) # for debugging and analysis purposes
    final_x0s = None
    ...
    for t in range(T):
        ...
        action, order = action[:, 0:1], action[:, 1:2] # Split the action into the actual alpha increment and the order selection (if using a single action head for both)
        next_obs, rewards, dones = env.step(action.squeeze(-1), order.squeeze(-1)) # Assuming action shape is (B, 1), squeeze to (B,)
        ...
        b_order_actions[t] = order # Store order action without the last dimension
        b_logprobs[t] = logprob
        b_rewards[t] = rewards
        b_dones[t] = dones
        b_values[t] = value
        
        b_action_means[t] = action_mean[:, 0:1]
        b_action_log_stds[t] = action_log_std[:, 0:1]

        b_order_means[t] = action_mean[:, 1:2]
        b_order_log_stds[t] = action_log_std[:, 1:2]
    ...
    b_order_actions = b_order_actions[:last_t]
    ...
    b_order_means = b_order_means[:last_t]  
    b_order_log_stds = b_order_log_stds[:last_t]
    ...
    rollout = {key: val[active_mask] for key, val in {
        ...
        'actions': b_actions, 'order_actions': b_order_actions, 'logprobs': b_logprobs, 'advantages': b_advantages,
        'returns': b_returns, 'dones': b_dones, 'values': b_values,
        'action_means': b_action_means, 'action_stds': torch.exp(b_action_log_stds),
        'order_means': b_order_means, 'order_stds': torch.exp(b_order_log_stds),
    }.items()}
...
def ppo_update(ppo_agent, minibatch_size=64, full_rollout=None):
    for i in range(0, full_rollout['states'].shape[0], minibatch_size):
        ...
        actions_and_order = torch.cat([minibatch['actions'], minibatch['order_actions']], dim=-1)
        new_logprobs, new_values, entropy, new_action_mean, new_action_log_std = ppo_agent.evaluate_actions(minibatch['states'], minibatch['alphas'], minibatch['steps'], actions_and_order)
...
if __name__ == "__main__":
    ...
    ppo_exp_suffix = f"{args.dataset}_NFE_{args.budget}_Nthorder"
